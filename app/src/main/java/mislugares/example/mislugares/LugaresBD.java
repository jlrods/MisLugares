...
/**
 * Created by rodjose1 on 23/04/2018.
 */
...
public class LugaresBD extends SQLiteOpenHelper implements Lugares {
    ...
    public void borrar(int id) {
        ...
        //bd.execSQL("DELETE FROM lugares WHERE tipo = " + TipoLugar.OTROS.ordinal());
        bd.close();
    }
    ...
    public int buscarPorNombre(String nombre){
        ...
        int id = -1;
        SQLiteDatabase bd = getReadableDatabase();
        Cursor cursor = bd.rawQuery("SELECT _id FROM lugares WHERE nombre = " + nombre,
            null);
        if (cursor.moveToNext()) {
            ...
            Toast.makeText(contexto, "Sin el permiso, no se puede tomar la foto.", Toast.LENGTH_SHORT).show();
            return id;
        }
        cursor.close();
        bd.close();
